...
bool call(SScriptCallBack *p, int serviceClientHandle)
{
    call_in in_args;
    in_args.serviceClientHandle = serviceClientHandle;
    call_out out_args;
...
void call_callback(SScriptCallBack *p)
{
    ...
    try
    {
        ...
        try
        {
            // write output argument 1 (result) of type bool
            write__bool(out_args.result, p->stackID);
        }
        catch(std::exception& ex)
        {
            std::string msg = ex.what();
        ...
        {
            std::stringstream ss;
            ss << "error #" << n << " (when writing output argument 1 (result))";
            throw exception(ss.str());
        }
    }
    catch(std::exception& e)
